...
struct ImageView: View {
    ...
    // New state to control which parser is used
    @State private var parseAsTimetable = false
    ...
    var body: some View {
            ...
            NavigationStack {
                VStack {
                    ...
                    HStack {
                        ...
                        // Conditionally show View Contacts button
                        ...
                        // Conditionally show View Timetable button
                        if !viewModel.timetableEntries.isEmpty {
                            Spacer()
                            NavigationLink("View Timetable") {
                                TimetableView(slots: viewModel.timeSlots, entries: viewModel.timetableEntries)
                            }.buttonStyle(RoundedButton())
                        }
                    ...
                    // Toggle to select parsing mode
                    Toggle("Parse as Timetable", isOn: $parseAsTimetable)
                        .padding(.horizontal)
                        .tint(.blue)
                        .onChange(of: parseAsTimetable) { _, newValue in
                            // Update the view model's flag
                            viewModel.parseAsTimetable = newValue
                            // Re-run recognition when the toggle changes
                            Task {
        ...
        .task {
            // Set the parsing mode on the view model *before* running recognition
            viewModel.parseAsTimetable = self.parseAsTimetable
